...
Dev tests: `python -m pytest tests/ -q` (needs `requirements-dev.txt`). No lint config in-repo.
...
- `scripts/cdr_full_sweep_v2.py` derives `REPO_ROOT` from the script location, so it works in any checkout. It regenerates committed `docs/` + `data/` outputs; a cold cache-warm run is ~33 min, a full `--refresh` is ~100 min.
