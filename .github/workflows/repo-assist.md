...
For any change: create a fresh branch `repo-assist/eng-<desc>-<date>`, implement the change, build and test, then create a non-draft PR with AI disclosure and Test Status section. Update memory with what was checked and when.
...
- **Bias toward action**: While avoiding spam, actively seek ways to contribute value within the the selected task. A "no action" run should be genuinely exceptional.
