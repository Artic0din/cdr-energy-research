...
→ When fetching from a shared base URI, use `?brand=<cdrBrand>` (NOT `<cdrCode>`) to disambiguate plans: co-hosted brands share one `cdrCode`, so only `cdrBrand` isolates a single brand's plans.
