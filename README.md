...
└── cache/                         gitignored, never committed; the v2 sweep writes to /tmp/cdr-cache (ephemeral, ~111.5 MB after a full v2 run; regenerate with scripts/cdr_full_sweep_v2.py)
```
...
`{base}` is `cdr.energymadeeasy.gov.au/<cdrCode>`. AER PDF is the authoritative source. From the committed `data/eme-refdata.json`, **3 base URIs are SHARED across multiple brands (9 brands total)**: `energy-locals` hosts Indigo / Cooperative / RAA / Energy Locals; `ovo-energy` hosts OVO Energy + MYOB OVO + OVO Energy CTM; `future-x` hosts Sunswitch + Future X. Co-hosted brands share one `cdrCode` but each has a distinct `cdrBrand`, so disambiguate with `?brand=<cdrBrand>` — filtering by `cdrCode` returns every co-hosted brand's plans.
